...
```bash
dotnet add package YAMCqrs.BackgroundWorker
```

## 📋 Descripción

Esta libreria proporciona una infraestructura robusta para implementar servicios en segundo plano que procesan elementos en lotes. La librería ofrece:
...
- **IWorkerStorage**: Storage para guardar el historial de ejecuciones. viene con una implementacion InMemory pero no se recomienda usarla para ambientes productivos.
...
// Registrar tu worker personalizado
...
El `CleanBackGroundWorker` se ejecuta cada 5 minutos para limpiar ejecuciones antiguas según tu configuración.
