...
using YAMCqrs.ServiceBus.Core.Configuration;
using YAMCqrs.ServiceBus.Core.SubscribeEvents.Abstractions;
using YAMCqrs.ServiceBus.Core.SubscribeEvents.Domain;

namespace YAMCqrs.ServiceBus.Core.SubscribeEvents.Implementation;
    ...
    ITopicToCommand topicToCommand) : YABackgroundWorker<SubscribeStoredEvent>(serviceProvider)
{
    ...
    private readonly ServiceBusConfiguration _config = config.Value ?? new ServiceBusConfiguration(); //TODO: delete the new a have a DI inyection.
